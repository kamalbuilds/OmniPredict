...
interface MarketCardProps {
  ...
  getSourceIcon: (source?: MarketSource) => JSX.Element | null;
}
...
export function MarketCard({ index, filter, getSourceIcon }: MarketCardProps) {
  ...
  return (
    <Card key={index} className="flex flex-col bg-gray-300">
      {isLoadingMarketData ? (
        <MarketCardSkeleton />
      ) : (
        <>
