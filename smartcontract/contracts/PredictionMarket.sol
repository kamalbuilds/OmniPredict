...
contract PredictionMarket is Ownable, ReentrancyGuard {
    ...
    struct Market {
        ...
        string category;
        string[] tags;
        uint256 marketFee; // Fee percentage (in basis points, e.g., 100 = 1%)
        uint256 validationPeriod; // Time window for disputing results
        uint256 totalVolume;
        bool disputed;
        mapping(address => uint256) userReputationPoints;
    }
    ...
    // New state variables
    bool public paused;
    uint256 public defaultMarketFee = 100; // 1% default fee
    ...
    event MarketValidated(uint256 indexed marketId);
     ...
     * @param _tags Array of tags for the market.
     * @param _marketFee The fee percentage for the market.
    ...
    function createMarket(
        ...
        require(_marketFee <= 500, "Fee too high"); // Max 5% fee
        ...
        market.tags = _tags;
        market.marketFee = _marketFee > 0 ? _marketFee : defaultMarketFee;
        market.validationPeriod = 3 days;
    ...
    // Function to dispute market outcome
    function disputeMarketOutcome(uint256 _marketId) external {
        Market storage market = markets[_marketId];
        require(market.resolved, "Market not resolved");
        require(
            block.timestamp <= market.endTime + market.validationPeriod,
    ...
    function updateUserReputation(address user, uint256 points) internal {
        userReputation[user] += points;
        emit ReputationUpdated(user, userReputation[user]);
    }

    // New function to set valid categories
    function setValidCategory(string memory category, bool isValid) external onlyOwner {
